...
model_string <- textConnection(
  ...
  for (i in 1:n_ind){
    ...
    for (t in (f[i]+1):nocc){
      # State process
      z[i,t] ~ dbern(mu_phi * z[i,t-1])#[i,t-1]
      # Observation process
      y[i,t] ~ dbern(mu[i,t])
      mu[i,t] <- mu_p * z[i,t]#[i,t-1]
    } #t
  } #i
...
transformed data {
  int<lower=0,upper=n_occ+1> first[n_ind];     // first[i]: ind i first capture
  int<lower=0,upper=n_occ+1> last[n_ind];      // last[i]:  ind i last capture
  int<lower=0,upper=n_ind> n_captured[n_occ];  // n_capt[k]: num aptured at k

  first = rep_array(n_occ+1,n_ind);
  last = rep_array(0,n_ind);
  for (i in 1:n_ind) {
    for (k in 1:n_occ) {
      if (CH[i,k] == 1) {
        if (k < first[i]) 
